...
async function inserir(produto) {
    ...
    const res = await cliente.query('INSERT INTO produtos(nome,preco,idcategoria) VALUES ($1,$2,$3) RETURNING *', 
        [produto.nome, produto.preco, produto.idcategoria]);
    await cliente.end();
    return res.rows[0]
}
...
async function atualizar(id, produto) {
    ...
    const res = await cliente.query('UPDATE produtos SET nome=$1, preco=$2, idcategoria=$3 WHERE id=$4 RETURNING *', 
        [produto.nome, produto.preco, produto.idcategoria, id]);
    await cliente.end();
    return res.rows[0]
}
